...
## 진행 상황
🏗️ Phase 1(동역학+자세제어) 완료 — 6DOF 강체 동역학, PID 자세제어, 그리고 게인
튜닝 과정을 문서화한 폐루프 안정화 시뮬레이션을 구현하고 unit/integration
테스트 27개로 검증했음. 모터 믹싱·actuator saturation 진행 중.
상태추정과 요구사항 기반 V&V 하네스 개발 예정.
...
### 계획 (아직 구현 안 됨)
- 폐루프에 모터 믹싱·actuator saturation 반영 (진행 중)
...
## 검증 하네스 (계획)
아직 구현 안 됨 — 예정 설계 계획. 여기서 "결정론적"이란 LLM이 아니라 pytest 같은
일반 코드가 실행/판정을 맡는다는 뜻이다(같은 입력엔 항상 같은 결과). 시나리오
지표가 기준을 만족할 때까지 아래 루프를 반복한다:
